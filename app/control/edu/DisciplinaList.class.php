...
class DisciplinaList extends TPage
{
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_search_Disciplina');
        ...
        $nome->addValidation( 'Disciplina', new TRequiredValidator );

        $row = $this->form->addFields( [ new TLabel('ID'), $id ],
                                       [ new TLabel('Disciplina'), $nome ]);
        ...
        $this->form->addActionLink(_t('New'), new TAction(['DisciplinaForm', 'onEdit']), 'fa:plus green');
        ...
        $column_ordem = new TDataGridColumn('ordem', 'Ordem no Relatório', 'right');
        ...
        $action1 = new TDataGridAction(['DisciplinaForm', 'onEdit'], ['id'=>'{id}']);
        //$action2 = new TDataGridAction([$this, 'onDelete'], ['id'=>'{id}']);

        $action2 = new TDataGridAction(['DisciplinaAulasForm', 'onEdit']);
        //$action2->setDisplayCondition( array($this, 'matricular') );
        $action2->setField('id');
        
        $this->datagrid->addAction($action1, _t('Edit'),   'far:edit blue');
        $this->datagrid->addAction($action2, 'Adicionar Dias', 'fas:chalkboard-teacher red');
    ...
    public function onSearch()
    {
        ...
        TSession::setValue(__CLASS__.'_filter_ordem',   NULL);
        ...
        if (isset($data->ordem) AND ($data->ordem)) {
            $filter = new TFilter('ordem', 'like', "%{$data->ordem}%"); // create the filter
            TSession::setValue(__CLASS__.'_filter_ordem',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if (TSession::getValue(__CLASS__.'_filter_ordem')) {
                $criteria->add(TSession::getValue(__CLASS__.'_filter_ordem')); // add the session filter
            }
