<?php
/**
 * ContaReceberListHoje Listing
 * @author  <your name here>
 */
class ContaReceberListHoje extends TPage
{
    private $form; // form
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('ContaReceberListHoje_filter_descricao',   NULL);
        TSession::setValue('ContaReceberListHoje_filter_data_vencimento',   NULL);
        TSession::setValue('ContaReceberListHoje_filter_valor',   NULL);
        TSession::setValue('ContaReceberListHoje_filter_cliente_id',   NULL);
        TSession::setValue('ContaReceberListHoje_filter_tipo_pgto_id',   NULL);
        TSession::setValue('ContaReceberListHoje_filter_pc_receita_id',   NULL);
        TSession::setValue('ContaReceberListHoje_filter_conta_bancaria_id',   NULL);

        if (isset($data->descricao) AND ($data->descricao)) {
            $filter = new TFilter('descricao', 'like', "%{$data->descricao}%"); // create the filter
            TSession::setValue('ContaReceberListHoje_filter_descricao',   $filter); // stores the filter in the session
        }
        ...
        if (isset($data->data_vencimento) AND ($data->data_vencimento)) {
            ...
            TSession::setValue('ContaReceberListHoje_filter_data_vencimento',   $filter); // stores the filter in the session
        }


        if (isset($data->valor) AND ($data->valor)) {
            $filter = new TFilter('valor', 'like', "%{$data->valor}%"); // create the filter
            TSession::setValue('ContaReceberListHoje_filter_valor',   $filter); // stores the filter in the session
        }


        if (isset($data->cliente_id) AND ($data->cliente_id)) {
            $filter = new TFilter('cliente_id', '=', "$data->cliente_id"); // create the filter
            TSession::setValue('ContaReceberListHoje_filter_cliente_id',   $filter); // stores the filter in the session
        }


        if (isset($data->tipo_pgto_id) AND ($data->tipo_pgto_id)) {
            $filter = new TFilter('tipo_pgto_id', '=', "$data->tipo_pgto_id"); // create the filter
            TSession::setValue('ContaReceberListHoje_filter_tipo_pgto_id',   $filter); // stores the filter in the session
        }


        if (isset($data->pc_receita_id) AND ($data->pc_receita_id)) {
            $filter = new TFilter('pc_receita_id', '=', "$data->pc_receita_id"); // create the filter
            TSession::setValue('ContaReceberListHoje_filter_pc_receita_id',   $filter); // stores the filter in the session
        }
        ...
        if (isset($data->conta_bancaria_id) AND ($data->conta_bancaria_id)) {
            ...
            TSession::setValue('ContaReceberListHoje_filter_conta_bancaria_id',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if (TSession::getValue('ContaReceberListHoje_filter_descricao')) {
                $criteria->add(TSession::getValue('ContaReceberListHoje_filter_descricao')); // add the session filter
            }


            if (TSession::getValue('ContaReceberListHoje_filter_data_vencimento')) {
                $criteria->add(TSession::getValue('ContaReceberListHoje_filter_data_vencimento')); // add the session filter
            }


            if (TSession::getValue('ContaReceberListHoje_filter_valor')) {
                $criteria->add(TSession::getValue('ContaReceberListHoje_filter_valor')); // add the session filter
            }


            if (TSession::getValue('ContaReceberListHoje_filter_cliente_id')) {
                $criteria->add(TSession::getValue('ContaReceberListHoje_filter_cliente_id')); // add the session filter
            }


            if (TSession::getValue('ContaReceberListHoje_filter_tipo_pgto_id')) {
                $criteria->add(TSession::getValue('ContaReceberListHoje_filter_tipo_pgto_id')); // add the session filter
            }


            if (TSession::getValue('ContaReceberListHoje_filter_pc_receita_id')) {
                $criteria->add(TSession::getValue('ContaReceberListHoje_filter_pc_receita_id')); // add the session filter
            }


            if (TSession::getValue('ContaReceberListHoje_filter_conta_bancaria_id')) {
                $criteria->add(TSession::getValue('ContaReceberListHoje_filter_conta_bancaria_id')); // add the session filter
            }
                ...
                foreach ($objects as $object)
                {
                    ...
                    if ($total == $atual){
                        ...
                        $script = TScript::create('$(document).ready(function () {
                            ...
                            __adianti_ajax_exec(\'class=ContaReceberListHoje&method=onSelect&id=\'+code+\'&check=\'+value_check);
                            });});',false);
