<?php
class RelFluxoCaixaGeral 
{
    private $arquivo = "/tmp/RelFluxoCaixaGeralTodas.pdf";

    public function __construct($param)
    {
        $this->onGerarRelFluxoCaixaGeral($param);
    }
    ...
    public function onGerarRelFluxoCaixaGeral($param)
    {
        ...
        try
        {
            ...
            $pdf->Cell(275,5,"FLUXO DE CAIXA GERAL (TODAS AS CONTAS)",0,0,'C', true);

            //$conta = new ContaBancaria($contaBancaria);

            $pdf->SetXY(10, 45);
            $pdf->SetFont('helvetica','B',9);
            $pdf->Cell(275,5,"Dados de Todas as Contas Bancárias juntas.",0,0,'L');
            ...
            $pdf->SetXY(175, 60);
            $pdf->Cell(20,5,"Conta",0,0,'L', true);
            ...
            LEFT JOIN conta_bancaria cb ON (cb.id = movimentacao_bancaria.conta_bancaria_id)
            LEFT JOIN banco b ON (b.id = cb.banco_id)
            WHERE movimentacao_bancaria.data_baixa between ? and ? and movimentacao_bancaria.deleted_at is null
            ...
            foreach ($resultListagem as $dado) {
                ...
                $pdf->Cell(15,5,$dataformat,0,0,'C');

                $pdf->Cell(15,5,$dado['id'],0,0,'L');
                
                if($dado['entrada'] != 0.00){
                $pdf->SetTextColor(5,110,10);
                $pdf->SetFont('helvetica','',7);
                $pdf->Cell(70,5, substr($dado['cliente'],0,45),0,0,'L');
                }else{
                $pdf->SetTextColor(248,57,8);
                $pdf->SetFont('helvetica','',7);
                $pdf->Cell(70,5,substr($dado['fornecedor'],0,45),0,0,'L');
                }

                $pdf->Cell(65,5,substr($dado['pc'],0,45),0,0,'L');

                $pdf->Cell(20,5,substr($dado['conta'],0,14),0,0,'L');

                $pdf->Cell(30,5,$dado['pgto'],0,0,'L');
