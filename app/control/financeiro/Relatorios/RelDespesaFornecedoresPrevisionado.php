<?php
class RelDespesaFornecedoresPrevisionado 
{
    private $arquivo = "/tmp/RelDespesaFornecedoresPrevisionado.pdf";

    public function __construct($param)
    {
        $this->onGerarDespesaFornecedoresPrevisionado($param);
    }
    ...
    {   
        ...
        //$fornecedor_id = $param['fornecedor_idfor'];
        ...
        try
        {
            ...
            $pdf->Cell(190,5,"Resumo de despesas por fornecedores Projetado",0,0,'C', true);
   
            $conn = TTransaction::get();

            $unit_id = TSession::getValue('userunitid');

            $listagem = $conn->prepare("SELECT cp.fornecedor_id, f.nome_fantasia FROM conta_pagar cp INNER JOIN fornecedor f ON (f.id = cp.fornecedor_id) WHERE cp.data_vencimento BETWEEN ? AND ? AND cp.unit_id = ? GROUP BY 1 ORDER BY cp.data_vencimento"); 

            $listagem->execute(array($dataInicio,$dataFim,$unit_id));
            ...
            foreach ($resultlistagem as $result) {
                ...
                $listagem2 = $conn->prepare("SELECT data_vencimento, descricao, valor FROM conta_pagar WHERE fornecedor_id = ? AND data_vencimento BETWEEN ? AND ? AND unit_id = ? AND baixa = 'N' ORDER BY data_vencimento"); 
                ...
                foreach ($resultlistagem2 as $result2) {
                    ...
                    $data_vencimento = $result2['data_vencimento'];
