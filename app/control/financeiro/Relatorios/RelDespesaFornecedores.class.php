<?php
class RelDespesaFornecedores 
{
    private $arquivo = "/tmp/RelDespesaFornecedores.pdf";

    public function __construct($param)
    {
        $this->onGerarDespesaFornecedores($param);
    }
    ...
    {   
        ...
        try
        {
            ...
            $pdf->Cell(190,5,"Resumo de despesas por fornecedores",0,0,'C', true);
   
            $conn = TTransaction::get();

            $unit_id = TSession::getValue('userunitid');

            $listagem = $conn->prepare("SELECT cp.fornecedor_id, f.nome_fantasia FROM conta_pagar cp INNER JOIN fornecedor f ON (f.id = cp.fornecedor_id) WHERE cp.data_baixa BETWEEN ? AND ? AND cp.unit_id = ? GROUP BY 1 ORDER BY cp.data_baixa"); 
