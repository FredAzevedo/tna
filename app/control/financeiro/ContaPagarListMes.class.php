<?php
/**
 * ContaPagarListMes Listing
 * @author  Fred Az.
 */
class ContaPagarListMes extends TPage
{
    private $form; // form
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('ContaPagarListMes_filter_descricao',   NULL);
        TSession::setValue('ContaPagarListMes_filter_documento',   NULL);
        TSession::setValue('ContaPagarListMes_filter_data_vencimento',   NULL);
        TSession::setValue('ContaPagarListMes_filter_valor',   NULL);
        TSession::setValue('ContaPagarListMes_filter_fornecedor_id',   NULL);
        TSession::setValue('ContaPagarListMes_filter_pc_despesa_id',   NULL);
        TSession::setValue('ContaPagarListMes_filter_conta_bancaria_id',   NULL);

        if (isset($data->descricao) AND ($data->descricao)) {
            $filter = new TFilter('descricao', 'like', "%{$data->descricao}%"); // create the filter
            TSession::setValue('ContaPagarListMes_filter_descricao',   $filter); // stores the filter in the session
        }


        if (isset($data->documento) AND ($data->documento)) {
            $filter = new TFilter('documento', 'like', "%{$data->documento}%"); // create the filter
            TSession::setValue('ContaPagarListMes_filter_documento',   $filter); // stores the filter in the session
        }


        if (isset($data->data_vencimento) AND ($data->data_vencimento)) {
            $filter = new TFilter('data_vencimento', '=', "NOESC:date(MONTH(NOW())"); // create the filter
            TSession::setValue('ContaPagarListMes_filter_data_vencimento',   $filter); // stores the filter in the session
        }


        if (isset($data->valor) AND ($data->valor)) {
            $filter = new TFilter('valor', 'like', "%{$data->valor}%"); // create the filter
            TSession::setValue('ContaPagarListMes_filter_valor',   $filter); // stores the filter in the session
        }


        if (isset($data->fornecedor_id) AND ($data->fornecedor_id)) {
            $filter = new TFilter('fornecedor_id', '=', "$data->fornecedor_id"); // create the filter
            TSession::setValue('ContaPagarListMes_filter_fornecedor_id',   $filter); // stores the filter in the session
        }


        if (isset($data->pc_despesa_id) AND ($data->pc_despesa_id)) {
            $filter = new TFilter('pc_despesa_id', '=', "$data->pc_despesa_id"); // create the filter
            TSession::setValue('ContaPagarListMes_filter_pc_despesa_id',   $filter); // stores the filter in the session
        }
        ...
        if (isset($data->conta_bancaria_id) AND ($data->conta_bancaria_id)) {
            ...
            TSession::setValue('ContaPagarListMes_filter_conta_bancaria_id',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if (TSession::getValue('ContaPagarListMes_filter_descricao')) {
                $criteria->add(TSession::getValue('ContaPagarListMes_filter_descricao')); // add the session filter
            }


            if (TSession::getValue('ContaPagarListMes_filter_documento')) {
                $criteria->add(TSession::getValue('ContaPagarListMes_filter_documento')); // add the session filter
            }


            if (TSession::getValue('ContaPagarListMes_filter_data_vencimento')) {
                $criteria->add(TSession::getValue('ContaPagarListMes_filter_data_vencimento')); // add the session filter
            }


            if (TSession::getValue('ContaPagarListMes_filter_valor')) {
                $criteria->add(TSession::getValue('ContaPagarListMes_filter_valor')); // add the session filter
            }


            if (TSession::getValue('ContaPagarListMes_filter_fornecedor_id')) {
                $criteria->add(TSession::getValue('ContaPagarListMes_filter_fornecedor_id')); // add the session filter
            }


            if (TSession::getValue('ContaPagarListMes_filter_pc_despesa_id')) {
                $criteria->add(TSession::getValue('ContaPagarListMes_filter_pc_despesa_id')); // add the session filter
            }


            if (TSession::getValue('ContaPagarListMes_filter_conta_bancaria_id')) {
                $criteria->add(TSession::getValue('ContaPagarListMes_filter_conta_bancaria_id')); // add the session filter
            }
                ...
                foreach ($objects as $object)
                {
                    ...
                    if ($total == $atual){
                        ...
                        $script = TScript::create('$(document).ready(function () {
                            ...
                            __adianti_ajax_exec(\'class=ContaPagarListMes&method=onSelect&id=\'+code+\'&check=\'+value_check);
                            });});',false);
