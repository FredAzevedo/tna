<?php
/**
 * EstoqueRequisicaoForm Master/Detail
 * @author  Fred Azv.
 */
class EstoqueRequisicaoForm extends TPage
{
    protected $form; // form
    ...
    public function __construct()
    {
        ...
        $user_id->addValidation('Técnico Requerente', new TRequiredValidator);
        ...
        $row = $this->form->addFields( [ new TLabel('Produto requerido'), $detail_produto_id ],
        ...
        $this->detail_list->setId('EstoqueRequisicao_list');
        ...
        ////$container->add(new TXMLBreadCrumb('menu.xml', 'EstoqueRequisicaoList'));
    ...
    public static function onEditDetail( $param )
    {
        ...
        // fill detail fields
        TForm::sendData( 'form_EstoqueRequisicao', $data );
    }
    ...
    public static function onDeleteDetail( $param )
    {
        ...
        TScript::create("ttable_remove_row_by_id('EstoqueRequisicao_list', '{$detail_id}')");
    }
    ...
    public function onEdit($param)
    {
        try
        {
            ...
            if (isset($param['key']))
            {
                ...
                $object = new EstoqueRequisicao($key);
                $items  = EstoqueRequisicaoItens::where('estoque_requisicao_id', '=', $key)->load();
    ...
    public function onSave()
    {
        try
        {
            ...
            $data = $this->form->getData();
            $master = new EstoqueRequisicao;
            $master->fromArray( (array) $data);
            $this->form->validate(); // form validation
            
            $master->store(); // save master object
            // delete details
            $old_items = EstoqueRequisicaoItens::where('estoque_requisicao_id', '=', $master->id)->load();
            ...
            if( $items )
            {
                foreach( $items as $item )
                {
                    ...
                    {
                        $detail = new EstoqueRequisicaoItens;
                    }
                    else
                    {
                        $detail = EstoqueRequisicaoItens::find($item['id']);
                    }
                    $detail->produto_id  = $item['produto_id'];
                    $detail->quantidade  = $item['quantidade'];
                    $detail->estoque_requisicao_id = $master->id;
                    $detail->store();
