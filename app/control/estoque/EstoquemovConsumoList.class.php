<?php
/**
 * EstoquemovConsumoList Listing
 * @author  Fred Azv.
 */
class EstoquemovConsumoList extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_EstoquemovConsumo');
        $this->form->setFormTitle('Inventário de Consumo (Lançamentos de movimentação em estoque de Consumo)');
        ...
        $this->form->addActionLink('Fazer Lançamento', new TAction(['EstoquemovConsumoForm', 'onEdit']), 'fa:plus green');
    ...
    public function onInlineEdit($param)
    {
        try
        {
            ...
            $object = new EstoquemovConsumo($key); // instantiates the Active Record
            $object->{$field} = $value;
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('EstoquemovConsumoList_filter_unit_id',   NULL);
        TSession::setValue('EstoquemovConsumoList_filter_produto_id',   NULL);
        TSession::setValue('EstoquemovConsumoList_filter_local',   NULL);
        TSession::setValue('EstoquemovConsumoList_filter_tipo',   NULL);
        TSession::setValue('EstoquemovConsumoList_filter_quantidade',   NULL);
        TSession::setValue('EstoquemovConsumoList_filter_saldo',   NULL);
        TSession::setValue('EstoquemovConsumoList_filter_valor',   NULL);
        TSession::setValue('EstoquemovConsumoList_filter_referencia',   NULL);
        TSession::setValue('EstoquemovConsumoList_filter_updated_at',   NULL);
        ...
        if (isset($data->unit_id) AND ($data->unit_id)) {
            ...
            TSession::setValue('EstoquemovConsumoList_filter_unit_id',   $filter); // stores the filter in the session
        }


        if (isset($data->produto_id) AND ($data->produto_id)) {
            $filter = new TFilter('produto_id', '=', "$data->produto_id"); // create the filter
            TSession::setValue('EstoquemovConsumoList_filter_produto_id',   $filter); // stores the filter in the session
        }


        if (isset($data->local) AND ($data->local)) {
            $filter = new TFilter('local', 'like', "%{$data->local}%"); // create the filter
            TSession::setValue('EstoquemovConsumoList_filter_local',   $filter); // stores the filter in the session
        }


        if (isset($data->tipo) AND ($data->tipo)) {
            $filter = new TFilter('tipo', '=', "$data->tipo"); // create the filter
            TSession::setValue('EstoquemovConsumoList_filter_tipo',   $filter); // stores the filter in the session
        }


        if (isset($data->quantidade) AND ($data->quantidade)) {
            $filter = new TFilter('quantidade', 'like', "%{$data->quantidade}%"); // create the filter
            TSession::setValue('EstoquemovConsumoList_filter_quantidade',   $filter); // stores the filter in the session
        }


        if (isset($data->saldo) AND ($data->saldo)) {
            $filter = new TFilter('saldo', 'like', "%{$data->saldo}%"); // create the filter
            TSession::setValue('EstoquemovConsumoList_filter_saldo',   $filter); // stores the filter in the session
        }


        if (isset($data->valor) AND ($data->valor)) {
            $filter = new TFilter('valor', 'like', "%{$data->valor}%"); // create the filter
            TSession::setValue('EstoquemovConsumoList_filter_valor',   $filter); // stores the filter in the session
        }


        if (isset($data->referencia) AND ($data->referencia)) {
            $filter = new TFilter('referencia', 'like', "%{$data->referencia}%"); // create the filter
            TSession::setValue('EstoquemovConsumoList_filter_referencia',   $filter); // stores the filter in the session
        }


        if (isset($data->updated_at) AND ($data->updated_at)) {
            $filter = new TFilter('updated_at', 'like', "%{$data->updated_at}%"); // create the filter
            TSession::setValue('EstoquemovConsumoList_filter_updated_at',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            $repository = new TRepository('EstoquemovConsumo');
            $limit = 10;
            // creates a criteria
            ...
            if (TSession::getValue('EstoquemovConsumoList_filter_unit_id')) {
                $criteria->add(TSession::getValue('EstoquemovConsumoList_filter_unit_id')); // add the session filter
            }


            if (TSession::getValue('EstoquemovConsumoList_filter_produto_id')) {
                $criteria->add(TSession::getValue('EstoquemovConsumoList_filter_produto_id')); // add the session filter
            }


            if (TSession::getValue('EstoquemovConsumoList_filter_local')) {
                $criteria->add(TSession::getValue('EstoquemovConsumoList_filter_local')); // add the session filter
            }


            if (TSession::getValue('EstoquemovConsumoList_filter_tipo')) {
                $criteria->add(TSession::getValue('EstoquemovConsumoList_filter_tipo')); // add the session filter
            }


            if (TSession::getValue('EstoquemovConsumoList_filter_quantidade')) {
                $criteria->add(TSession::getValue('EstoquemovConsumoList_filter_quantidade')); // add the session filter
            }


            if (TSession::getValue('EstoquemovConsumoList_filter_saldo')) {
                $criteria->add(TSession::getValue('EstoquemovConsumoList_filter_saldo')); // add the session filter
            }


            if (TSession::getValue('EstoquemovConsumoList_filter_valor')) {
                $criteria->add(TSession::getValue('EstoquemovConsumoList_filter_valor')); // add the session filter
            }


            if (TSession::getValue('EstoquemovConsumoList_filter_referencia')) {
                $criteria->add(TSession::getValue('EstoquemovConsumoList_filter_referencia')); // add the session filter
            }


            if (TSession::getValue('EstoquemovConsumoList_filter_updated_at')) {
                $criteria->add(TSession::getValue('EstoquemovConsumoList_filter_updated_at')); // add the session filter
            }
    ...
    public static function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('sample'); // open a transaction with database
            $object = new EstoquemovConsumo($key, FALSE); // instantiates the Active Record
