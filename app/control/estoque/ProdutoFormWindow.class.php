 ...
 * @author  Fred Azv.
 */
class ProdutoFormWindow extends TWindow
{
    protected $form; // form
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_ProdutoWindow');
        ...
        $orig = new TDBCombo('orig', 'sample', 'NfeIcmsOrigem', 'id', 'descricao');
        ...
        $impostos_venda = new TDBUniqueSearch('impostos_venda', 'sample', 'NfeRegra', 'id', 'nome');
        $impostos_compra = new TDBUniqueSearch('impostos_compra', 'sample', 'NfeRegra', 'id', 'nome');
        ...
        $det_quantidade = new TEntry('det_quantidade');
        $det_quantidade->setMask('9!');
        ...
        $this->det_list->addColumn( new TDataGridColumn('valor_unidade', 'Valor Unitário', 'right', 100) );
        $this->det_list->addColumn( new TDataGridColumn('quantidade', 'Quantidade', 'right', 100) );
        $this->det_list->addColumn( new TDataGridColumn('valor_total', 'Valor Total', 'right', 100) );
        ...
        $detail_markup_preco_venda = new TNumeric('detail_markup_preco_venda', 2, ',', '.', true);
        $detail_markup_preco_venda->setEditable(FALSE);
        $detail_markup_comissao_tecnico = new TNumeric('detail_markup_comissao_tecnico', 2, ',', '.', true);
        $detail_markup_comissao_parceiro = new TNumeric('detail_markup_comissao_parceiro', 2, ',', '.', true);
        ...
        $row = $this->form->addFields( [ new TLabel('Preço do markup'), $detail_markup_preco_venda ]);
        $row->layout = ['col-sm-2'];
        ...
        TScript::create('markup = function() {
                ...
                let valorComposicao;
                valorComp = form_ProdutoWindow.det_quantidade.value * parseFloat(form_ProdutoWindow.det_valor_unidade.value);
                valorComposicao= formatMoney(valorComp);
                form_ProdutoWindow.det_valor_total.value = valorComposicao;

                let markup;    
                markup = 100/[100 -(
                    parseFloat(form_ProdutoWindow.detail_markup_despesa_variavel.value) +
                    parseFloat(form_ProdutoWindow.detail_markup_despesa_fixa.value) +
                    parseFloat(form_ProdutoWindow.detail_markup_lucro_desejado.value) +
                    parseFloat(form_ProdutoWindow.detail_markup_comissao_tecnico.value) +
                    parseFloat(form_ProdutoWindow.detail_markup_comissao_parceiro.value)

                )];

                let valorMarkup = 
                parseFloat(form_ProdutoWindow.detail_markup_preco_custo.value) * markup;

                 let valor = formatMoney(valorMarkup);
                 form_ProdutoWindow.detail_markup_preco_venda.value = valor;
            };
    ...
    public function onDetailAddComposicao( $param )
    {
        try
        {
            ...
            $grid_data['valor_unidade'] = $data->det_valor_unidade;
            $grid_data['quantidade'] = $data->det_quantidade;
            $grid_data['valor_total'] = $data->det_valor_total;
            ...
            TForm::sendData( 'form_ProdutoWindow', $data, false, false );
        }
        catch (Exception $e)
        {
    ...
    public static function onDetailEditComposicao( $param )
    {
        ...
        $data->det_quantidade = $param['quantidade'];
        $data->det_valor_unidade = $param['valor_unidade'];
        $data->det_valor_total = $param['valor_total'];
            ...
            try
             {
                ...
                TTransaction::close();
                TDBCombo::reloadFromModel('form_ProdutoWindow', 'produto_subgrupo_id', 'sample','ProdutoSubgrupo','id','nome','', $criteria);
    ...
    public static function onEditDetail( $param )
    {
        ...
        // fill detail fields
        TForm::sendData( 'form_ProdutoWindow', $data );
    }
        ...
        try
        {
            ...
            if( $param['ProdutoComposicao_list_composicao_id'] )
            {
                foreach( $param['ProdutoComposicao_list_composicao_id'] as $key => $item_id )
                {
                    ...
                    $detail->quantidade  = $param['ProdutoComposicao_list_quantidade'][$key];
                    $detail->valor_unidade  = $param['ProdutoComposicao_list_valor_unidade'][$key];
                    $detail->valor_total  = $param['ProdutoComposicao_list_valor_total'][$key];
                    $detail->produto_id = $master->id;
                    $detail->store();
                }
            }
