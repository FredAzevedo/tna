...
class EstoqueList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle('Estoque de Produtos');
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('EstoqueList_filter_produto_id',   NULL);
        //TSession::setValue('EstoqueList_filter_local',   NULL);
        TSession::setValue('EstoqueList_filter_saldo',   NULL);
        TSession::setValue('EstoqueList_filter_updated_at',   NULL);
        ...
        if (isset($data->produto_id) AND ($data->produto_id)) {
            ...
            TSession::setValue('EstoqueList_filter_produto_id',   $filter); // stores the filter in the session
        }


        /*if (isset($data->local) AND ($data->local)) {
            $filter = new TFilter('local', 'like', "%{$data->local}%"); // create the filter
            TSession::setValue('EstoqueList_filter_local',   $filter); // stores the filter in the session
        }*/


        if (isset($data->saldo) AND ($data->saldo)) {
            $filter = new TFilter('saldo', 'like', "%{$data->saldo}%"); // create the filter
            TSession::setValue('EstoqueList_filter_saldo',   $filter); // stores the filter in the session
        }


        if (isset($data->updated_at) AND ($data->updated_at)) {
            $filter = new TFilter('updated_at', 'like', "%{$data->updated_at}%"); // create the filter
            TSession::setValue('EstoqueList_filter_updated_at',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if (TSession::getValue('EstoqueList_filter_produto_id')) {
                $criteria->add(TSession::getValue('EstoqueList_filter_produto_id')); // add the session filter
            }


            if (TSession::getValue('EstoqueList_filter_local')) {
                $criteria->add(TSession::getValue('EstoqueList_filter_local')); // add the session filter
            }


            if (TSession::getValue('EstoqueList_filter_saldo')) {
                $criteria->add(TSession::getValue('EstoqueList_filter_saldo')); // add the session filter
            }


            if (TSession::getValue('EstoqueList_filter_updated_at')) {
                $criteria->add(TSession::getValue('EstoqueList_filter_updated_at')); // add the session filter
            }
