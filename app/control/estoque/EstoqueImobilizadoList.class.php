<?php
/**
 * EstoqueImobilizadoList Listing
 * @author  Fred Azv.
 */
class EstoqueImobilizadoList extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_EstoqueImobilizado');
        $this->form->setFormTitle('Estoque de Imobilizados');
        ...
        $action1 = new TDataGridAction(array('EstoqueControlImobilizadoList', 'onReload'));
        $action1->setLabel('Listagem');
        $action1->setImage('fas:search');
        $action1->setField('produto_id');

        $action2 = new TDataGridAction(array('EstoqueControlmovImobilizadoForm', 'getProduto'));
        $action2->setLabel('Emplacamento');
        $action2->setImage('fas:car');
        $action2->setField('produto_id');

        $action_group = new TDataGridActionGroup(' Ações', 'fas:cog');

        $action_group->addHeader('Operações');
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('EstoqueImobilizadoList_filter_produto_id',   NULL);
        //TSession::setValue('EstoqueImobilizadoList_filter_local',   NULL);
        TSession::setValue('EstoqueImobilizadoList_filter_saldo',   NULL);
        TSession::setValue('EstoqueImobilizadoList_filter_updated_at',   NULL);
        ...
        if (isset($data->produto_id) AND ($data->produto_id)) {
            ...
            TSession::setValue('EstoqueImobilizadoList_filter_produto_id',   $filter); // stores the filter in the session
        }


        /*if (isset($data->local) AND ($data->local)) {
            $filter = new TFilter('local', 'like', "%{$data->local}%"); // create the filter
            TSession::setValue('EstoqueImobilizadoList_filter_local',   $filter); // stores the filter in the session
        }*/


        if (isset($data->saldo) AND ($data->saldo)) {
            $filter = new TFilter('saldo', 'like', "%{$data->saldo}%"); // create the filter
            TSession::setValue('EstoqueImobilizadoList_filter_saldo',   $filter); // stores the filter in the session
        }


        if (isset($data->updated_at) AND ($data->updated_at)) {
            $filter = new TFilter('updated_at', 'like', "%{$data->updated_at}%"); // create the filter
            TSession::setValue('EstoqueImobilizadoList_filter_updated_at',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            $repository = new TRepository('EstoqueImobilizado');
            $limit = 10;
            ...
            if (TSession::getValue('EstoqueImobilizadoList_filter_produto_id')) {
                $criteria->add(TSession::getValue('EstoqueImobilizadoList_filter_produto_id')); // add the session filter
            }


            if (TSession::getValue('EstoqueImobilizadoList_filter_local')) {
                $criteria->add(TSession::getValue('EstoqueImobilizadoList_filter_local')); // add the session filter
            }


            if (TSession::getValue('EstoqueImobilizadoList_filter_saldo')) {
                $criteria->add(TSession::getValue('EstoqueImobilizadoList_filter_saldo')); // add the session filter
            }


            if (TSession::getValue('EstoqueImobilizadoList_filter_updated_at')) {
                $criteria->add(TSession::getValue('EstoqueImobilizadoList_filter_updated_at')); // add the session filter
            }
    ...
    public static function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('sample'); // open a transaction with database
            $object = new EstoqueImobilizado($key, FALSE); // instantiates the Active Record
